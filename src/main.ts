...
const replyCurrentSettings = async (interaction: ChatInputCommandInteraction<CacheType>) => {
  ...
  await interaction.reply({ embeds: [settingEmbed] });
};
...
const replyHelp = async (interaction: ChatInputCommandInteraction<CacheType>) => {
  const helpEmbed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "設定チャンネル",
        value: "決められたフォーマットで設定を記述すると各設定を変更できます。",
      },
      {
        name: "VCログの別チャンネル表示",
        value:
          "権限を限定したチャンネルにログを送信したい等のため、「secret」というチャンネルにもログを送信できます。\n`secretChannel:`に続けてのIDを書き込みます。複数ある場合は半角スペースを挟んで連続して設定できます。",
        inline: true,
      },
      {
        name: "VCログ非表示設定",
        value:
          "VCログを表示しないVCを設定できます`hideVC:`に続けてIDを書き込みます。複数ある場合は半角スペースを挟んで連続して設定できます。",
        inline: true,
      },
      {
        name: "forumタグ追加",
        value:
          "forumチャンネルのタグを追加するコマンドです。\n`/addtags inputname: タグ名 selectchannel: チャンネル名`\nタグ名は半角スペースを区切りとして複数同時に登録できます。",
      },
    );
...
const commandsInfoList: commandsList[] = [
  {
    ...
    func: async (interaction: ChatInputCommandInteraction<CacheType>) =>
      await replyMsg(interaction, "pong"),
  },
  {
    name: "hoge",
    discription: "hogehogeを返すよ",
    func: async (interaction: ChatInputCommandInteraction<CacheType>) =>
      await replyMsg(interaction, "hogehoge"),
  },
  {
    name: "setting",
    discription: "設定の確認",
    func: async (interaction: ChatInputCommandInteraction<CacheType>) =>
      await replyCurrentSettings(interaction),
  },
  {
    name: "help",
    discription: "botの使い方",
...
// コマンドビルダを配列化
const commands = commandsInfoList.map((info) =>
  new SlashCommandBuilder().setName(info.name).setDescription(info.discription).toJSON(),
);
...
// タグ追加のビルダ作成
const addTagsBuilder = new SlashCommandBuilder()
  .setName("addtags")
  .setDescription("forumにタグを追加")
  .addStringOption((option) =>
    option.setName("tagname").setDescription("tag name").setRequired(true),
  )
...
guildIDs.forEach((guildId) => {
  if (!clientId || !guildId) {
    console.error("clientId or guildId is not defined. Please check your environment variables.");
    return;
  }
  rest
    .put(Routes.applicationGuildCommands(clientId, guildId), {
      body: [...commands, addTagsBuilder],
    })
    .then((data: any) => {
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "addtags") {
    ...
    await forumChannel.setAvailableTags(
      [
        ...availableTags,
        ...inputTagsAry.map((tag) => {
          return { name: tag };
        }),
      ],
      "set",
    );
    const msg = `${forumChannel.name} に\`${inputTagsAry.join(", ")}\`を追加しました。`;
    await interaction.reply(msg);
    console.log(msg);
  }
});
...
// 入出ログ出力メソッド
const sendInOutMsg = async (oldState: VoiceState, newState: VoiceState, secretVC: string[]) => {
  // vc-noticeのチャンネルIDを取得
  const VcNoticeID =
    oldState.guild.channels.cache.find((channel) => channel.name === "vc-notice")?.id ?? "";
  ...
  if (oldState.channel?.name) {
    ...
    if (secretVcIdsList.includes(oldState.channel?.id)) {
      const secretChannel = oldState.guild.channels.cache.find(
        (channel) => channel.name === "secret",
      ) as TextChannel;
      if (!secretChannel) return;
      await secretChannel.send(outMsg);
    } else {
  ...
  if (newState.channel?.name) {
    ...
    if (secretVcIdsList.includes(newState.channel?.id)) {
      const secretChannel = newState.guild.channels.cache.find(
        (channel) => channel.name === "secret",
      ) as TextChannel;
      if (!secretChannel) return;
      await secretChannel.send(intoMsg);
    } else {
      await channel.send(intoMsg);
    }
  }
};
...
client.on("voiceStateUpdate", async (oldState, newState) => {
  const settingChannel = oldState.guild.channels.cache.find(
    (channel) => channel.name === "settings",
  ) as TextChannel;
  ...
  if (oldState.channel?.name !== newState.channel?.name) {
    ...
    if (!settings.hideVC.includes((oldState.channel?.id ?? "") || (newState.channel?.id ?? ""))) {
      await sendInOutMsg(oldState, newState, settings.secretChannel);
    } else {
